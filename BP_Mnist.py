...
def train(mnist):
    ...
    # 这个运算首先将一个布尔型的数值转化为实数型，然后计算平均值。这个平均值就是模型在这
    # 一组数据上的正确率
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            #返回有800个图片组成的训练集（800*784）和（800*100）的输出集
    ...
    with tf.Session() as sess:
        #加载图结构
        saver = tf.train.import_meta_graph('./minist_model/MODEL-29001.meta')
        # 加载检查点文件checkpoint，包括w1和w2，具体到文件夹即可
        saver.restore(sess, tf.train.latest_checkpoint('./minist_model'))
